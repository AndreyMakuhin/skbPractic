		...
		public static void DrawDragonFractal(Pixels pixels, int iterationsCount, int seed)
		{
            ...
            for (int i = 0; i < iterationsCount; i++)
            {
                ...
                if (rnd.Next(2) ==  0)
                {
                    ...
                    x = ((x * Math.Cos(Math.PI *45/180) - y * Math.Sin(Math.PI*45 / 180)) / Math.Sqrt(2));
                    y = ((oldX * Math.Sin(Math.PI *45/ 180) + y * Math.Cos(Math.PI *45/ 180)) / Math.Sqrt(2));
                }
                else
                {
                    x = ( (x * Math.Cos(Math.PI* 135 /180) - y * Math.Sin(Math.PI* 135 /180) )/ Math.Sqrt(2) ) + 1;
                    y = ( (oldX * Math.Sin(Math.PI *135/ 180) + y * Math.Cos(Math.PI *135/ 180)) / Math.Sqrt(2) );
                }
